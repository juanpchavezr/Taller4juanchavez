...
// codigo que se modifica para utilizar potenciometros
void drawRacket() {
 // if (0 < port.available())
{
  //leer = port.read();
  ...
  //println(leer);
  //mapeadoY= map(leer,500,0,500,0);
  //mapeadoX= map(leer,0,500,0,500);
  //fill(racketColor);
rectMode(CENTER);
  posY=mapeadoY;
  posX=mapeadoX;
  rect(posX, posY, racketWidth, racketHeight,5);
  }
}
